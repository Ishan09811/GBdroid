...
import io.github.gbdroid.databinding.FragmentBiosManagerBinding
...
class BiosManagerFragment : Fragment() {
    ...
    private fun updatePlatformUi(platform: Platform, isLoaded: Boolean, filename: String = "") {
        ...
        if (isLoaded) {
            ...
            val colorOnSurface = MaterialColors.getColor(requireView(), R.attr.colorOnSurfaceVariant)
            statusView.setTextColor(colorOnSurface)
            button.text = "Replace"
        } else {
            statusView.text = "Missing"
            val colorError = MaterialColors.getColor(requireView(), R.attr.colorOnError)
